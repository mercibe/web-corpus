...
public class JpaMotManager implements MotManager
{
    ...
    {
        return (Mot) entityManager.createQuery("select m from Mot m where m.mot = ?").setParameter(1, mot)
                .getSingleResult();
    }
    ...
    public List<Mot> findByGraphie(String graphie, Condition condition, FiltreMot filtres)
    {
        ...
        else
        {
            ...
            p = cb.conjunction();
            p = cb.and(p, cb.like(motRacine.get("lemme").as(String.class), likeCondition));

        }

        // TODO ajouter les filtres au prédicat
        if (filtres != null)
        {
            p = addFiltresToPrédicat(motRacine, p, filtres);
        }
    ...
    private Predicate addFiltresToPrédicat(Root<Mot> motRacine, Predicate p, FiltreMot filtres)
    {
        //TODO implémenter  Quid si 
        
        System.err.println("Gérer les filtres: " + filtres);
