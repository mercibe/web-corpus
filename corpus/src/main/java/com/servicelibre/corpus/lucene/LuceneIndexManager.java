...

{
    ...
    public RésultatRecherche getDocumentsWithContexts(String query, BooleanQuery booleanQuery, int slop, int window)
    {
        RésultatRecherche result = new RésultatRecherche();

        // TODO idéalement le query devrait être parsé par un Query Parser «amélioré» (SpanNearQuery à la place d'un PhraseQuery
        ...
        if (query != null && query.trim().length() > 0)
        {
            //FIXME créer/étendre un QueryParser spécifique pour s'assurer d'utiliser l'analyseur utilisé pour l'indexation!!!
        ...
        else if (hasBooleanQuery && hasContextualQuery)
        {
            ...
            //SpanQueryFilter spanQueryFilter = new SpanQueryFilter(spanNearQuery);
        ...
        {
            /*
             * I think maybe you could reverse this around.  Get a filter from your BooleanQuery and get the DocIdSet 
             * and then advance through the Spans and the DocIdSetIterator, as they will both be forward facing.  
             * For each span, check to see whether that doc is in the filter or not.
             * http://lucene.apache.org/java/3_0_1/api/contrib-queries/org/apache/lucene/search/BooleanFilter.html
                                DocIdSet docIdSet = bQueryFilter.getDocIdSet(reader);
                                DocIdSetIterator docIdSetIterator = docIdSet.iterator();
                                int docId;
                                List<Integer> docs = new ArrayList<Integer>(scoreDocs.length);
            ...
            try
            {
                ...
                if (hasBooleanQuery)
                {
                    // filtrer les spans conserver uniquement ceux des docs retournés par le query
                ...
                while (spans.next() == true)
                {
                    ...
                    if (!hasBooleanQuery || docs.contains(spans.doc()))
                    {
                        ...
                        if (oldDoc != spans.doc())
                        {
                            System.err.println("Traitement du document " + spans.doc());

                            // extraire le voisinage avec un TermVectorMapper (PositionBasedTermVectorMapper)
                            try
                            {

                                reader.getTermFreqVector(spans.doc(), DEFAULT_SEARCH_FIELD, tvm);
                                txtField = reader.document(spans.doc(), new MapFieldSelector(DEFAULT_SEARCH_FIELD))
                            ...
                            System.out.println("after getTermFreqVector - " + tokenPositions);
                        }
                        oldDoc = spans.doc();
                        ...
                        //        List<TermVectorOffsetInfo> startOffsets = tvPositionInfo.getOffsets();
                        //        System.out.println("start offsetinfo : " + startOffsets);
                        //        for (TermVectorOffsetInfo termVectorOffsetInfo : startOffsets)
                        //        {
                        //            System.out.println(termVectorOffsetInfo.getStartOffset() + "=>" + termVectorOffsetInfo.getEndOffset());   
                        //        }
                    ...
                    //
                    //                        // FIXME lent...
                    //                        System.out.println("before getContexte");
            ...
            PositionBasedTermVectorMapper tvm, int window)
    {
        ...
        System.out.println("before getTermFreqVector");
        ...
        System.out.println("after getTermFreqVector");
