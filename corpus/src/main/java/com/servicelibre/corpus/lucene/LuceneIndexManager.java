...

{
	 ...
	 * @return
	 */
	public RésultatRecherche getDocumentsWithContexts(String query, int slop, int window) {
			...
			while (spans.next() == true) {
				...
				if (oldDoc != currentDocId) {
					...
					try {
						...
						txtField = reader.document(currentDocId, new MapFieldSelector(DEFAULT_SEARCH_FIELD)).getField(DEFAULT_SEARCH_FIELD).stringValue();
