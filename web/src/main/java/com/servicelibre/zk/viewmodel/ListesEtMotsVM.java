...
public class ListesEtMotsVM {
	...
	if (listeSélectionné != null && listeSélectionné.getId() != 0) {
	    ...
	    f.addFiltre(new Filtre(CléFiltre.liste.name() + "_" + listeSélectionné.getCatégorie().getNom(), listeSélectionné.getCatégorie().getNom(),
		    new Long[] { listeSélectionné.getId() }));
