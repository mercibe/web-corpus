...
e)});c.p.footerrow&&a(c.grid.sDiv).children("td:eq("+b+")").css("display",e);h=this.widthOrg?this.widthOrg:parseInt(this.width,10);if(e==="none")c.p.tblwidth-=h+g;else c.p.tblwidth+=h+g;this.hidden=!j;f=true}});if(f===true)if(c.p.shrinkToFit===false){a("table:first",c.grid.bDiv).css("width",c.p.tblwidth+"px");a("table:first",c.grid.hDiv).css("width",c.p.tblwidth+"px");c.grid.hDiv.scrollLeft=c.grid.bDiv.scrollLeft;c.p.footerrow&&a("table:first",c.grid.sDiv).css("width",c.p.tblwidth+"px")}else c.grid.width!==
c.p.tblwidth&&a(c).jqGrid("setGridWidth",c.p.tblwidth)}})},hideCol:function(d){return this.each(function(){a(this).jqGrid("showHideCol",d,"none")})},showCol:function(d){return this.each(function(){a(this).jqGrid("showHideCol",d,"")})},remapColumns:function(d,e,c){function f(j){var b;b=j.length?a.makeArray(j):a.extend({},j);a.each(d,function(p){j[p]=b[this]})}function g(j,b){a(">tr"+(b||""),j).each(function(){var p=this,k=a.makeArray(p.cells);a.each(d,function(){var n=k[this];n&&p.appendChild(n)})})}
var h=this.get(0);f(h.p.colModel);f(h.p.colNames);f(h.grid.headers);g(a("thead:first",h.grid.hDiv),c&&":not(.ui-jqgrid-labels)");e&&g(a("#"+a.jgrid.jqID(h.p.id)+" tbody:first"),".jqgfirstrow, tr.jqgrow, tr.jqfoot");h.p.footerrow&&g(a("tbody:first",h.grid.sDiv));if(h.p.remapColumns)if(h.p.remapColumns.length)f(h.p.remapColumns);else h.p.remapColumns=a.makeArray(d);h.p.lastsort=a.inArray(h.p.lastsort,d);if(h.p.treeGrid)h.p.expColInd=a.inArray(h.p.expColInd,d)},setGridWidth:function(d,e){return this.each(function(){if(this.grid){var c=
this,f,g=0,h=a.browser.webkit||a.browser.safari?0:c.p.cellLayout,j,b=0,p=false,k=c.p.scrollOffset,n,l=0,o=0,m;if(typeof e!="boolean")e=c.p.shrinkToFit;if(!isNaN(d)){d=parseInt(d,10);c.grid.width=c.p.width=d;a("#gbox_"+a.jgrid.jqID(c.p.id)).css("width",d+"px");a("#gview_"+a.jgrid.jqID(c.p.id)).css("width",d+"px");a(c.grid.bDiv).css("width",d+"px");a(c.grid.hDiv).css("width",d+"px");c.p.pager&&a(c.p.pager).css("width",d+"px");c.p.toppager&&a(c.p.toppager).css("width",d+"px");if(c.p.toolbar[0]===true){a(c.grid.uDiv).css("width",
d+"px");c.p.toolbar[1]=="both"&&a(c.grid.ubDiv).css("width",d+"px")}c.p.footerrow&&a(c.grid.sDiv).css("width",d+"px");if(e===false&&c.p.forceFit===true)c.p.forceFit=false;if(e===true){a.each(c.p.colModel,function(){if(this.hidden===false){f=this.widthOrg?this.widthOrg:parseInt(this.width,10);g+=f+h;if(this.fixed)l+=f+h;else b++;o++}});if(b!==0){c.p.tblwidth=g;n=d-h*b-l;if(!isNaN(c.p.height))if(a(c.grid.bDiv)[0].clientHeight<a(c.grid.bDiv)[0].scrollHeight||c.rows.length===1){p=true;n-=k}g=0;var v=
c.grid.cols.length>0;a.each(c.p.colModel,function(q){if(this.hidden===false&&!this.fixed){f=this.widthOrg?this.widthOrg:parseInt(this.width,10);f=Math.round(n*f/(c.p.tblwidth-h*b-l));if(!(f<0)){this.width=f;g+=f;c.grid.headers[q].width=f;c.grid.headers[q].el.style.width=f+"px";if(c.p.footerrow)c.grid.footers[q].style.width=f+"px";if(v)c.grid.cols[q].style.width=f+"px";j=q}}});m=0;if(p){if(d-l-(g+h*b)!==k)m=d-l-(g+h*b)-k}else if(Math.abs(d-l-(g+h*b))!==1)m=d-l-(g+h*b);c.p.colModel[j].width+=m;c.p.tblwidth=
g+m+h*b+l;if(c.p.tblwidth>d){p=c.p.tblwidth-parseInt(d,10);c.p.tblwidth=d;f=c.p.colModel[j].width-=p}else f=c.p.colModel[j].width;c.grid.headers[j].width=f;c.grid.headers[j].el.style.width=f+"px";if(v)c.grid.cols[j].style.width=f+"px";a("table:first",c.grid.bDiv).css("width",c.p.tblwidth+"px");a("table:first",c.grid.hDiv).css("width",c.p.tblwidth+"px");c.grid.hDiv.scrollLeft=c.grid.bDiv.scrollLeft;if(c.p.footerrow){c.grid.footers[j].style.width=f+"px";a("table:first",c.grid.sDiv).css("width",c.p.tblwidth+
"px")}}}}}})},setGridHeight:function(d){return this.each(function(){if(this.grid){a(this.grid.bDiv).css({height:d+(isNaN(d)?"":"px")});this.p.height=d;this.p.scroll&&this.grid.populateVisible()}})},setCaption:function(d){return this.each(function(){this.p.caption=d;a("span.ui-jqgrid-title",this.grid.cDiv).html(d);a(this.grid.cDiv).show()})},setLabel:function(d,e,c,f){return this.each(function(){var g=-1;if(this.grid)if(typeof d!="undefined"){a(this.p.colModel).each(function(b){if(this.name==d){g=
b;return false}});if(g>=0){var h=a("tr.ui-jqgrid-labels th:eq("+g+")",this.grid.hDiv);if(e){var j=a(".s-ico",h);a("[id^=jqgh_]",h).empty().html(e).append(j);this.p.colNames[g]=e}if(c)typeof c==="string"?a(h).addClass(c):a(h).css(c);typeof f==="object"&&a(h).attr(f)}}})},setCell:function(d,e,c,f,g,h){return this.each(function(){var j=-1,b,p;if(this.grid){if(isNaN(e))a(this.p.colModel).each(function(n){if(this.name==e){j=n;return false}});else j=parseInt(e,10);if(j>=0)if(b=this.rows.namedItem(d)){var k=
a("td:eq("+j+")",b);if(c!==""||h===true){b=this.formatter(d,c,j,b,"edit");p=this.p.colModel[j].title?{title:a.jgrid.stripHtml(b)}:{};this.p.treeGrid&&a(".tree-wrap",a(k)).length>0?a("span",a(k)).html(b).attr(p):a(k).html(b).attr(p);if(this.p.datatype=="local"){b=this.p.colModel[j];c=b.formatter&&typeof b.formatter==="string"&&b.formatter=="date"?a.unformat.date(c,b):c;p=this.p._index[d];if(typeof p!="undefined")this.p.data[p][b.name]=c}}if(typeof f==="string")a(k).addClass(f);else f&&a(k).css(f);
typeof g==="object"&&a(k).attr(g)}}})},getCell:function(d,e){var c=false;this.each(function(){var f=-1;if(this.grid){if(isNaN(e))a(this.p.colModel).each(function(j){if(this.name===e){f=j;return false}});else f=parseInt(e,10);if(f>=0){var g=this.rows.namedItem(d);if(g)try{c=a.unformat(a("td:eq("+f+")",g),{rowId:g.id,colModel:this.p.colModel[f]},f)}catch(h){c=a.jgrid.htmlDecode(a("td:eq("+f+")",g).html())}}}});return c},getCol:function(d,e,c){var f=[],g,h=0,j=0,b=0,p;e=typeof e!="boolean"?false:e;if(typeof c==
"undefined")c=false;this.each(function(){var k=-1;if(this.grid){if(isNaN(d))a(this.p.colModel).each(function(m){if(this.name===d){k=m;return false}});else k=parseInt(d,10);if(k>=0){var n=this.rows.length,l=0;if(n&&n>0){for(;l<n;){if(a(this.rows[l]).hasClass("jqgrow")){try{g=a.unformat(a(this.rows[l].cells[k]),{rowId:this.rows[l].id,colModel:this.p.colModel[k]},k)}catch(o){g=a.jgrid.htmlDecode(this.rows[l].cells[k].innerHTML)}if(c){p=parseFloat(g);h+=p;j=Math.min(j,p);b=Math.max(j,p)}else e?f.push({id:this.rows[l].id,
value:g}):f.push(g)}l++}if(c)switch(c.toLowerCase()){case "sum":f=h;break;case "avg":f=h/n;break;case "count":f=n;break;case "min":f=j;break;case "max":f=b}}}}});return f},clearGridData:function(d){return this.each(function(){if(this.grid){if(typeof d!="boolean")d=false;if(this.p.deepempty)a("#"+a.jgrid.jqID(this.p.id)+" tbody:first tr:gt(0)").remove();else{var e=a("#"+a.jgrid.jqID(this.p.id)+" tbody:first tr:first")[0];a("#"+a.jgrid.jqID(this.p.id)+" tbody:first").empty().append(e)}this.p.footerrow&&
d&&a(".ui-jqgrid-ftable td",this.grid.sDiv).html("&#160;");this.p.selrow=null;this.p.selarrrow=[];this.p.savedRow=[];this.p.records=0;this.p.page=1;this.p.lastpage=0;this.p.reccount=0;this.p.data=[];this.p_index={};this.updatepager(true,false)}})},getInd:function(d,e){var c=false,f;this.each(function(){if(f=this.rows.namedItem(d))c=e===true?f:f.rowIndex});return c},bindKeys:function(d){var e=a.extend({onEnter:null,onSpace:null,onLeftKey:null,onRightKey:null,scrollingRows:true},d||{});return this.each(function(){var c=
this;a("body").is("[role]")||a("body").attr("role","application");c.p.scrollrows=e.scrollingRows;a(c).keydown(function(f){var g=a(c).find("tr[tabindex=0]")[0],h,j,b,p=c.p.treeReader.expanded_field;if(g){b=c.p._index[g.id];if(f.keyCode===37||f.keyCode===38||f.keyCode===39||f.keyCode===40){if(f.keyCode===38){j=g.previousSibling;h="";if(j)if(a(j).is(":hidden"))for(;j;){j=j.previousSibling;if(!a(j).is(":hidden")&&a(j).hasClass("jqgrow")){h=j.id;break}}else h=j.id;a(c).jqGrid("setSelection",h)}if(f.keyCode===
40){j=g.nextSibling;h="";if(j)if(a(j).is(":hidden"))for(;j;){j=j.nextSibling;if(!a(j).is(":hidden")&&a(j).hasClass("jqgrow")){h=j.id;break}}else h=j.id;a(c).jqGrid("setSelection",h)}if(f.keyCode===37){c.p.treeGrid&&c.p.data[b][p]&&a(g).find("div.treeclick").trigger("click");a.isFunction(e.onLeftKey)&&e.onLeftKey.call(c,c.p.selrow)}if(f.keyCode===39){c.p.treeGrid&&!c.p.data[b][p]&&a(g).find("div.treeclick").trigger("click");a.isFunction(e.onRightKey)&&e.onRightKey.call(c,c.p.selrow)}return false}else if(f.keyCode===
13){a.isFunction(e.onEnter)&&e.onEnter.call(c,c.p.selrow);return false}else if(f.keyCode===32){a.isFunction(e.onSpace)&&e.onSpace.call(c,c.p.selrow);return false}}})})},unbindKeys:function(){return this.each(function(){a(this).unbind("keydown")})},getLocalRow:function(d){var e=false,c;this.each(function(){if(typeof d!=="undefined"){c=this.p._index[d];if(c>=0)e=this.p.data[c]}});return e}})})(jQuery);
...
a(G).jqGrid("getInd",E,true);if(ka){a("td",ka).each(function(ca){M=ea[ca].name;if(M!=="cb"&&M!=="subgrid"&&M!=="rn"&&ea[ca].editable===true){if(M==G.p.ExpandColumn&&G.p.treeGrid===true)X=a(this).text();else try{X=a.unformat(a(this),{rowId:E,colModel:ea[ca]},ca)}catch(ta){X=a(this).html()}if(m.p.autoencode)X=a.jgrid.htmlDecode(X);if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[M]=X;M=a.jgrid.jqID(M);switch(ea[ca].edittype){case "password":case "text":case "button":case "image":case "textarea":if(X==
"&nbsp;"||X=="&#160;"||X.length==1&&X.charCodeAt(0)==160)X="";a("#"+M,"#"+U).val(X);break;case "select":var oa=X.split(",");oa=a.map(oa,function(u){return a.trim(u)});a("#"+M+" option","#"+U).each(function(){this.selected=!ea[ca].editoptions.multiple&&(oa[0]==a.trim(a(this).text())||oa[0]==a.trim(a(this).val()))?true:ea[ca].editoptions.multiple?a.inArray(a.trim(a(this).text()),oa)>-1||a.inArray(a.trim(a(this).val()),oa)>-1?true:false:false});break;case "checkbox":X+="";if(ea[ca].editoptions&&ea[ca].editoptions.value)if(ea[ca].editoptions.value.split(":")[0]==
X){a("#"+M,"#"+U).attr("checked",true);a("#"+M,"#"+U).attr("defaultChecked",true)}else{a("#"+M,"#"+U).attr("checked",false);a("#"+M,"#"+U).attr("defaultChecked","")}else{X=X.toLowerCase();if(X.search(/(false|0|no|off|undefined)/i)<0&&X!==""){a("#"+M,"#"+U).attr("checked",true);a("#"+M,"#"+U).attr("defaultChecked",true)}else{a("#"+M,"#"+U).attr("checked",false);a("#"+M,"#"+U).attr("defaultChecked","")}}break;case "custom":try{if(ea[ca].editoptions&&a.isFunction(ea[ca].editoptions.custom_value))ea[ca].editoptions.custom_value(a("#"+
M,"#"+U),"set",X);else throw"e1";}catch(xa){xa=="e1"?a.jgrid.info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.nodefined,jQuery.jgrid.edit.bClose):a.jgrid.info_dialog(jQuery.jgrid.errors.errcap,xa.message,jQuery.jgrid.edit.bClose)}}Q++}});Q>0&&a("#id_g","#"+r).val(E)}}}function j(){var E,G=[true,"",""],U={},M=m.p.prmNames,Q,X,Y,ha;if(a.isFunction(d.beforeCheckValues)){var Z=d.beforeCheckValues(C,a("#"+q),C[m.p.id+"_id"]=="_empty"?M.addoper:M.editoper);if(Z&&typeof Z===
"object")C=Z}for(Y in C)if(C.hasOwnProperty(Y)){G=a.jgrid.checkValues(C[Y],Y,m);if(G[0]===false)break}if(G[0]){if(a.isFunction(d.onclickSubmit))U=d.onclickSubmit(d,C)||{};if(a.isFunction(d.beforeSubmit))G=d.beforeSubmit(C,a("#"+q))}if(G[0]&&!d.processing){d.processing=true;a("#sData","#"+r+"_2").addClass("ui-state-active");X=M.oper;Q=M.id;C[X]=a.trim(C[m.p.id+"_id"])=="_empty"?M.addoper:M.editoper;if(C[X]!=M.addoper)C[Q]=C[m.p.id+"_id"];else if(C[Q]===undefined)C[Q]=C[m.p.id+"_id"];delete C[m.p.id+
"_id"];C=a.extend(C,d.editData,U);if(m.p.treeGrid===true){if(C[X]==M.addoper){ha=a(m).jqGrid("getGridParam","selrow");C[m.p.treeGridModel=="adjacency"?m.p.treeReader.parent_id_field:"parent_id"]=ha}for(i in m.p.treeReader){U=m.p.treeReader[i];if(C.hasOwnProperty(U))C[X]==M.addoper&&i==="parent_id_field"||delete C[U]}}U=a.extend({url:d.url?d.url:a(m).jqGrid("getGridParam","editurl"),type:d.mtype,data:a.isFunction(d.serializeEditData)?d.serializeEditData(C):C,complete:function(ma,ea){if(ea!="success"){G[0]=
false;G[1]=a.isFunction(d.errorTextFormat)?d.errorTextFormat(ma):ea+" Status: '"+ma.statusText+"'. Error code: "+ma.status}else if(a.isFunction(d.afterSubmit))G=d.afterSubmit(ma,C);if(G[0]===false){a("#FormError>td","#"+r).html(G[1]);a("#FormError","#"+r).show()}else{a.each(m.p.colModel,function(){if(S[this.name]&&this.formatter&&this.formatter=="select")try{delete S[this.name]}catch(ta){}});C=a.extend(C,S);m.p.autoencode&&a.each(C,function(ta,oa){C[ta]=a.jgrid.htmlDecode(oa)});d.reloadAfterSubmit=
d.reloadAfterSubmit&&m.p.datatype!="local";if(C[X]==M.addoper){G[2]||(G[2]=a.jgrid.randId());C[Q]=G[2];if(d.closeAfterAdd){if(d.reloadAfterSubmit)a(m).trigger("reloadGrid");else if(m.p.treeGrid===true)a(m).jqGrid("addChildNode",G[2],ha,C);else{a(m).jqGrid("addRowData",G[2],C,c.addedrow);a(m).jqGrid("setSelection",G[2])}a.jgrid.hideModal("#"+s.themodal,{gb:"#gbox_"+v,jqm:c.jqModal,onClose:d.onClose})}else if(d.clearAfterAdd){if(d.reloadAfterSubmit)a(m).trigger("reloadGrid");else m.p.treeGrid===true?
a(m).jqGrid("addChildNode",G[2],ha,C):a(m).jqGrid("addRowData",G[2],C,c.addedrow);h("_empty",m,q)}else if(d.reloadAfterSubmit)a(m).trigger("reloadGrid");else m.p.treeGrid===true?a(m).jqGrid("addChildNode",G[2],ha,C):a(m).jqGrid("addRowData",G[2],C,c.addedrow)}else{if(d.reloadAfterSubmit){a(m).trigger("reloadGrid");d.closeAfterEdit||setTimeout(function(){a(m).jqGrid("setSelection",C[Q])},1E3)}else m.p.treeGrid===true?a(m).jqGrid("setTreeRow",C[Q],C):a(m).jqGrid("setRowData",C[Q],C);d.closeAfterEdit&&
a.jgrid.hideModal("#"+s.themodal,{gb:"#gbox_"+v,jqm:c.jqModal,onClose:d.onClose})}if(a.isFunction(d.afterComplete)){E=ma;setTimeout(function(){d.afterComplete(E,C,a("#"+q));E=null},500)}if(d.checkOnSubmit||d.checkOnUpdate){a("#"+q).data("disabled",false);if(d._savedData[m.p.id+"_id"]!="_empty")for(var ka in d._savedData)if(C[ka])d._savedData[ka]=C[ka]}}d.processing=false;a("#sData","#"+r+"_2").removeClass("ui-state-active");try{a(":input:visible","#"+q)[0].focus()}catch(ca){}}},a.jgrid.ajaxOptions,
d.ajaxEditOptions);if(!U.url&&!d.useDataProxy)if(a.isFunction(m.p.dataProxy))d.useDataProxy=true;else{G[0]=false;G[1]+=" "+a.jgrid.errors.nourl}if(G[0])d.useDataProxy?m.p.dataProxy.call(m,U,"set_"+m.p.id):a.ajax(U)}if(G[0]===false){a("#FormError>td","#"+r).html(G[1]);a("#FormError","#"+r).show()}}function b(E,G){var U=false,M;for(M in E)if(E[M]!=G[M]){U=true;break}return U}function p(){a.each(m.p.colModel,function(E,G){if(G.editoptions&&G.editoptions.NullIfEmpty===true)if(C.hasOwnProperty(G.name)&&
